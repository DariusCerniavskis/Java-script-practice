...
const getCoord = (position) => {
    return position * cursorObj.fieldBoxSize;
};
...
const nextCursorPosition = (isNearest, isStartingPosition) => {
    ...
    if (cursorObj.xDirection + cursorObj.yDirection) {
        ...
        if (tempDirect < 0) {
            // left
            newIndex = tempValPosIndex ? tempValPosIndex : tempValPosLenght;
        } else {
            // right
            newIndex =
                tempValPosIndex < tempValPosLenght - 1 ? tempValPosIndex : -1;
        }
        newIndex += tempDirect;
    ...
    if (!curSize) {
        ...
        if (cursorObj.isMovingAnimation && !isStartingPosition) {
            if (cursorObj.ownerNumber) {
                ...
                if (!isBorderOut) {
                    tempSquare = cursorObj.yNew;
                } else {
                    tempSquare =
                        cursorObj.yNew +
                        gameObj.fieldSize * cursorObj.yDirection;
                }
                distanceSquares = tempSquare - cursorObj.y;
            } else {

                isBorderOut = !!cursorObj.xBorderOut;
                if (!isBorderOut) {
                    tempSquare = cursorObj.xNew;
                } else {
                    tempSquare =
                        cursorObj.xNew +
                        gameObj.fieldSize * cursorObj.xDirection;
                }
                distanceSquares = tempSquare - cursorObj.x;
            }

        //speed and distance

            oneStep(distanceSquares);
...
const animateTo = (
    ...
    if (isOverflow) {
        ...
        object2.style.borderColor =cursorObj.color[isVerticaly ?1:0];
                ...
                if (isOverflow) {
                    if (currentSize > 1) {
                        ...
                        position = finishPosition;
                                             object.style.borderColor = cursorObj.color[isVerticaly ?1:0];
                        if (isVerticaly) {
...
const oneStep =  (distanceSquares) => {
    ...
    const movingSpeed = gameObj.cursorSpeed * distanceSquares;
    ...
    let overflowStartPosition = 0;
        ...
        if (currDir < 0) {
            ...
            if (isOverflow) {
                currPos = 0;
                overflowCurrPos = gameObj.fieldSize-1;
                newPos =0
            }else   {
                newPos = currPos +1;
            }
...
const createFieldLine = (isFirstTime) => {
    ...
    if (tempValuePositions.length > 1) {
        ...
        nextCursorPosition(true, isFirstTime);
...
const startGame = () => {
    ...
    // box size
    // change height
    cursorObj.fieldBoxSize = Math.floor(500 / gameObj.fieldSize);
    ...
    taskDone = changeBoxesGroup(
        ...
        `${cursorObj.fieldBoxSize}px`,
    );
    cursorObj.size = cursorObj.fieldBoxSize - 2 * gameObj.cursorBorder;
